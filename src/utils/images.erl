...
magic(<<(?M_MARK), (?M_SOI), (?M_MARK), (?M_EXIF), _Len:16, "Exif", 0, 0, _/binary>>) ->
  <<"jpeg">>;
magic(<<(?M_MARK), (?M_SOI), (?M_MARK), (?M_JFIF), _Len:16, "JFIF", _, _, _/binary>>) ->
  <<"jpeg">>;
magic(<<(?M_MARK), (?M_SOI), (?M_MARK), (?M_DQT), _/binary>>) ->
  <<"jpeg">>;
magic(<<(?M_MARK), (?M_SOI), (?M_MARK), (?M_DHT), _/binary>>) ->
  <<"jpeg">>;
magic(<<(?M_MARK), (?M_SOI), (?M_MARK), (?M_SOF0), _/binary>>) ->
  <<"jpeg">>;
magic(<<(?M_MARK), (?M_SOI), (?M_MARK), (?M_SOS), _/binary>>) ->
  <<"jpeg">>;
magic(<<(?M_MARK), (?M_SOI), (?M_MARK), (?M_COM), _/binary>>) ->
  <<"jpeg">>;
magic(<<(?M_MARK), (?M_SOI), (?M_MARK), (?M_EXIF), _/binary>>) ->
  <<"jpeg">>;
...
frame_to_web(Data) ->
  Format = cast:to_binary(magic(Data)),
  FrameB = base64:encode(Data),
  <<"data:image/", Format/binary, ";base64,", FrameB/binary>>.
...
get_jpeg_block(<<(?M_MARK), (?M_SOF0), _:3/binary,
    Height:2/big-unsigned-integer-unit:8, Width:2/big-unsigned-integer-unit:8, _Rest/binary>>) ->
  {Width, Height};
