...
color_to_escape({Mod, Color}) ->
    ?CSI((mod(Mod) + code(Color)));
...
try_callback_call(Module, Function, Args, DefRet) ->
    try apply(Module, Function, Args) catch
        error:undef:S ->
            case S of
                [{Module, Function, Args, _} | _] -> DefRet;
                _ -> erlang:raise(error, undef, S)
            end
    end.
